...
export class WalletController {
  ...
  createWallet = async (req: Request, res: Response) => {
    try {
      ...
      if (typeof name !== "string" || isNaN(Number(userId))) {
        return res.status(400).json({ error: "Invalid fields" });
      }
      const wallet = new WalletEntity(name, parseInt(userId), amount);
  ...
  getWalletsByUserId = async (req: Request, res: Response) => {
    ...
    const wallet = await this.walletRepository.getWalletsByUserId(
      parseInt(userId)
    );
    res.json(wallet);
  };
